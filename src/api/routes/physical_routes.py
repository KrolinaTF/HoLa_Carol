...
@router.post("/physical/query", response_model=PhysicalResponse)
async def process_physical_query(query: PhysicalQuery):
    session = get_session()
    try:
        # Guardar la consulta
        db_query = Query(
       ...
       # Guardar resultado
        ...
        session.rollback()
        logger.error(f"Error processing physical query: {str(e)}")
